...
class HostCardEmulatorService : HostApduService() {
    companion object {
        ...
        fun setTag(tag: String) {
            NDEF_URI = NdefMessage(createTextRecord("", tag, NDEF_ID))
        ...
        var NDEF_URI = NdefMessage(createTextRecord("", "", NDEF_ID))
        var NDEF_URI_BYTES = NDEF_URI.toByteArray()
        var NDEF_URI_LEN = fillByteArrayToFixedDimension(
                BigInteger.valueOf(NDEF_URI_BYTES.size.toLong()).toByteArray(),
                2,
        )
    }
    ...
    private val READ_CAPABILITY_CONTAINER_RESPONSE = byteArrayOf(
            ...
            0x7F.toByte(), 0xFE.toByte(), // Maximum NDEF file size of 65534 bytes
    ...
    override fun processCommandApdu(commandApdu: ByteArray, extras: Bundle?): ByteArray {
        ...
        if (commandApdu.sliceArray(0..1).contentEquals(NDEF_READ_BINARY)) {
            ...
            val length = commandApdu.sliceArray(4 until commandApdu.size).toHex().toInt(16)
